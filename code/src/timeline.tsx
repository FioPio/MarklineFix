...
class Group extends React.Component<GroupProps> {
  render(): React.ReactNode {
    ...
    const style = {
      backgroundColor: group["background-color"], // change 'background-color' to backgroundColor
    ...
    return (
      <div className="groups" style={style}>
        ...
        <ol>
          {
            group.events.map((line, index) => {
              return <Line key={index} line={line} min_date={min_date} />
            })
          }
        </ol>
      </div>
...
class Line extends React.Component<LineProps> {
  render () {
    ...
    const minDateValue = min_date ? Number(min_date) : Number(new Date());
    const line_start = calcLength(Number(date_start) - minDateValue) + offset_left;
    ...
    const style = {
      marginLeft: `${line_start}px`, // change 'margin-left' to marginLeft
      color: line["text-color"] || '',
    };
    const styleEvent = {
      width: `${line_length}px`,
      backgroundColor: line["background-color"] || '', // change 'background-color' to backgroundColor
    };
          ...
          <ol style={styleEvent}>
            {
              line.events.map((event, index) => {
                return <Event key={index} event={event} offset={curr_offset_left} />
              })
            }
          </ol>
          <time>{line.date}</time>
  ...
  render() {
    ...
    this._process(data.body, {
      "line:start": function(line: IEvent) {
        const date_start = new Date(line["date-start"]);
        const date_end = new Date(line["date-end"]);

        if (min_date === null || date_start < min_date) {
          min_date = date_start;
        }

        if (max_date === null || date_end > max_date) {
          max_date = date_end;
        }
      }
    });

    const minDateValue = min_date ? min_date : new Date();
    const first_year = minDateValue.getFullYear();
    const maxDateValue = max_date ? max_date : new Date();
    const last_year = max_date ?maxDateValue.getFullYear() + 2 : first_year + 2;
    const years = last_year - first_year + 3;
    this.max_width = years * year_width + 90;

    min_date = min_date ? new Date(first_year, 0, 1) : new Date();
          ...
          >
            {
              data.body.map((group, index) => {
                return <Group key={index} group={group} min_date={min_date} scrollLeft={this.state.scrollLeft} width={this.max_width} />;
              })
            }
          </div>
        </section>
